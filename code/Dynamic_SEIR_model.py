...
class Train_Dynamic_SEIR:
    ...
    'rateSI' (base rate 'beta' from S to E, default 0.05)    感染率
    'rateIR' (base rate 'gamma' from I to R, default 0.01)   康复率
    'rateAl' (base rate of isolation 'altha', from E to I, default 0.1)   发病率，潜伏期到患病
    """
    ...
    def plot_fitted_beta_R0(self, real_obs: pandas.core.frame.DataFrame):
        ...
        plt.title('Fitted Dynamic Contact Rate and Transmissiblity of 2019-nCoV over time', fontsize = 18)
        # Contact Rate       Transmissibility
        plt.legend(['Contact Rate', 'Transmissibility'], prop={'size': 16}, bbox_to_anchor=(0.5, 1.02),
    ...
    def plot_fitted_result(self, real_obs: pandas.core.frame.DataFrame):
        ...
        plt.xlabel('日期')            # 2020 Date
        plt.ylabel('人数')            # Population
        plt.title('模型拟合', fontsize = 20)   # Fitted value by Dynamic SEIR model
        plt.legend(['拟合感染者','真实感染者', '拟合潜伏者', '真实潜伏者'], prop={'size': 12}, bbox_to_anchor=(0.5, 1.02),
           ncol=4, fancybox=True, shadow=True)
        # Estimated Infected   Real Infected    Estimated Exposed   Real Exposed
        plt.show()
...
class dynamic_SEIR:
    ...
    def run(self, death_rate):
        ...
        for i in range(1, self.eons): # number of prediction days
            ...
            #print(Exposed[-1])
           # I_to_D = (Infected[-1] * self.rateIR / 10)
            I_to_R = (Infected[-1] * self.rateIR)      ###
            
            Susceptible.append(Susceptible[-1] )  # - S_to_E
            Exposed.append(Exposed[-1] + S_to_E - E_to_I )  # - E_to_I
            Infected.append(Infected[-1] + E_to_I - I_to_R / 2 )  ######
    ...
    def plot(self, title, ylabel, xlabel, starting_point):
        ...
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
        plt.legend(['Susceptible','Infected','Exposed','Removed', 'Heal', 'Death'], prop={'size': 15}, bbox_to_anchor=(0.5, 1.02), ncol=6, fancybox=True, shadow=True)
    ...
    def plot_noSuscep(self, title, ylabel, xlabel, starting_point):
        ...
        # 感染者  模型预测回复者  潜伏着  治愈  死亡
        # Infected','Removed','Exposed','Heal','Death
        plt.title(title, fontsize = 20)
        plt.show()    
    
########################
## Model evaluation ,灵敏度分析
########################
